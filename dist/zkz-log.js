/**
 * author: zkz
 * @createTime: 2024-11-25 16:57:36
 * @Description: 打印模块方法
 */
(function webpackUniversalModuleDefinition(root, factory) {
	if (typeof exports === 'object' && typeof module === 'object') module.exports = factory();
	else if (typeof define === 'function' && define.amd) define([], factory);
	else if (typeof exports === 'object') exports['log'] = factory();
			 ...
			 */
				const print = (text, title, type) => {
					...
					if (typeof title != 'string') {
						throw new Error(
							'Only one variable can be printed at a time,The second parameter only supports type is string,'
						);
					}
					logger(
						`%c ${title} `,
				...
				const line = function() {
					...
					logger(
						...
						`border:1px solid ${typeColor(
							'line'
						)}; padding: 1px; border-radius: 4px; background: ${typeColor('line')}; color: #919398;`,
...
`;
					/* 内容代码 */
					let title1 = '🌒 zkz-LOG ';
					let title2 = 'author: zkz';
					let myUrl = 'https://gitee.com/zuo-bufan';
					let bdate = new Date();
					let content = `
版 本 号：1.0.2 
编译日期：${bdate} 
版权声明：
1. ZKZ-LOG版权完全属于 "zkz".
2. ZKZ-LOG可以免费使用，npm下载即可：
	1）有问题请联系作者：qq:2605331201
    2）保留此版权信息在控制台输出
    3）此提示信息只会打印三次
我们保留对此版权信息的最终解释权.

🏠 官网:  ${myUrl}
📫 Email:  2605331201@qq.com
`;
